...
namespace zamren_management_system.Areas.Procurement.ApiControllers.PurchaseRequisition;

[ApiController]
[Authorize(Roles = "EMPLOYEE")]
[Area("Procurement")]
[Route("api/procurement/purchase-requisition")]
public class PurchaseRequisitionApiController : ControllerBase
{
    ...
    private readonly IPurchaseRequisitionRequestService _purchaseRequisitionRequestService;
    ...
    private readonly IPurchaseRequisitionRequestAttachmentService _purchaseRequisitionRequestAttachmentService;
    ...
    public PurchaseRequisitionApiController(ILogger<PurchaseRequisitionApiController> logger,
        ...
        IWorkflowService workflowService, IPurchaseRequisitionRequestService purchaseRequisitionRequestService,
        ...
        IPurchaseRequisitionRequestAttachmentService purchaseRequisitionRequestAttachmentService)
    {
        _logger = logger;
        ...
        _purchaseRequisitionRequestService = purchaseRequisitionRequestService;
        ...
        _purchaseRequisitionRequestAttachmentService = purchaseRequisitionRequestAttachmentService;
    }
...
public class WkfAttachmentDto
{
    ...
    [HttpPost("get-initiate-purchase-requisition-data")]
    ...
    public async Task<IActionResult> GetInitiatePurchaseRequisitionData()
    {
        try
        {
            ...
            PurchaseRequisitionRequestDto? purchaseRequisitionDto = null;
            ...
            if (!string.IsNullOrEmpty(reference) && !reference.Equals("null"))
            {
                ...
                var purchaseRequisition = await _purchaseRequisitionRequestService.FindByReferenceAsync(reference);
                if (purchaseRequisition == null)
                    return Ok(new { success = false, message = "Purchase requisition not found" });

                purchaseRequisitionDto = new PurchaseRequisitionRequestDto
                {
                ...
                var purchaseRequisitionRequestAttachments =
                    await _purchaseRequisitionRequestAttachmentService.FindByPurchaseRequisitionRequestIdAsync(
                        purchaseRequisition.Id);

                purchaseRequisitionDto.PurchaseRequisitionRequestAttachments = purchaseRequisitionRequestAttachments
                    .Select(attachment => new PurchaseRequisitionRequestAttachmentDto
                    {
                        Id = _cypherService.Encrypt(attachment.Id),
                        PlainId = attachment.Id,
                        PurchaseRequisitionRequest = purchaseRequisitionDto,
            ...
            //get process by ID
            var processId = ProcessConstant.PROC_PURCHASE_REQUISITION.GetProcessId()!;
            ...
            //get process 1st step by ID
            var currentStepId = ProcessStepConstant.STEP_1.GetProcessStepId()!;
            var currentStep = await _stepService.FindByIdAsync(currentStepId);
            if (currentStep == null)
    ...
    public async Task<IActionResult> SubmitPurchaseRequisitionForm()
    {
        try
        {
            var purchaseRequisitionDto = new PurchaseRequisitionRequestDto
            {
            ...
            var purchaseRequisition = new PurchaseRequisitionRequest
            {
            ...
            await _purchaseRequisitionRequestService.CreateAsync(purchaseRequisition);
            ...
            var workflowTask = await _workflowService.InitiateWorkflowTask(
                referenceNumber,
                currentUserId!,
                ProcessConstant.PROC_PURCHASE_REQUISITION.GetProcessId()!,
                comment,
                systemAttachments
            );
