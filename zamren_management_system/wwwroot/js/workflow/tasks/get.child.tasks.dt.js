$(document).ready(function () {
    window.ChildTasksDatatable = null;

    $('button[data-bs-toggle="tab"]').on('shown.bs.tab', function (e) {
        let target = $(e.target).attr('data-bs-target');
        if (target === '#tab3') {
            populateChildTasksDatatable();
        }
    });
    
    function populateChildTasksDatatable() {

        //if datatable already exists, destroy it
        if (window.ChildTasksDatatable) {
            window.ChildTasksDatatable.destroy();
        }
        ...
        ChildTasksDatatable = $("#child-tasks-table").DataTable({
            ajax: {
                url: "/api/workflow/task/get-child-tasks-dt",
                type: "POST",
                data: data,
                datatype: "json",
    ...
    function reloadTable() {
        ChildTasksDatatable.ajax.reload();
    }
});
